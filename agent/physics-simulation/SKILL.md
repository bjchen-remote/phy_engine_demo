...
- Point masses receive explicit acceleration fields, not world gravity. Orbits use mutual gravity; standalone spring/rod/rope scenes disable it and follow [connections](references/connections.md). Enable liquid/granular self-gravity only when the request explicitly asks for attraction caused by the particles' mass; read [particle-gravity](references/particle-gravity.md) first. For ordinary liquid collision or merging, keep `interactions.mutual_gravity=false` and use the requested initial velocities and liquid material properties. Never inflate `gravity_G` as a hidden cohesion or visual correction. Do not replace requested self-gravity with point masses or a prescribed radial field. Sand/water erosion is qualitative.
