...
First load, build, or author the complete scene and identify its target fluid entity ID. Use `physics_liquid` when adding a named liquid or changing an entity to water, honey, glue, lava, or molten lead, and apply its `patch_arguments.operations` with `physics_patch`. If an example already has the requested preset and explicit `properties`, preserve those properties: reapplying the generic preset would erase deliberate visual controls. Keep the explicit water material in the `droplet_ground`, `droplet_ground_micro_wet`, `droplet_ground_splash`, `droplet_ground_macro_wet` and `droplet_cone` examples unless the user asks for a material change. The macro wet scene gives both drop and film an uncalibrated `surface_tension=0.055` coefficient; it is not a direct molecular-force measurement. The liquid response contains no `scene_json`; if no scene exists yet, obtain one and call `physics_liquid` again before patching. Do not guess coefficients.
